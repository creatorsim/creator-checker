...
for T in $TEST_1; do
	...
	sed 's/ir\[at[^;]*;\s//g' | \
	sed 's/ir\[fp[^;]*;\s//g' | \
	sed 's/ir\[a[1-7][^;]*;\s//g' > solution/output/output_ejercicio1_$T.txt 
...
for T in $TEST_3; do
	...
	if [ -n "${exits}" ]
	then
		mem=$(sed 's/.*memory\[\([^:]*\)\]:[^;]*;.*/\1/' solution/output/output_ejercicio3_$T.txt | tail -n 2 | sed ':a;N;$!ba;s/\n//g')
		
		if [ $(($mem)) -le $((0x0FFFFFFF)) ] && [ $(($mem)) -ge $((0x05BBFCBF)) ]
		then
			sed "s/memory\[$mem\][^;]*;\s//g" solution/output/output_ejercicio3_$T.txt > solution/output/aux_output_ejercicio3_$T.txt
		else
			cp solution/output/output_ejercicio3_$T.txt solution/output/aux_output_ejercicio3_$T.txt
		fi
	else 
		cp solution/output/output_ejercicio3_$T.txt solution/output/aux_output_ejercicio3_$T.txt
	fi
	...
	#Delete registers, display and keyboard
	sed 's/cr[^;]*;\s//g' solution/output/aux_output_ejercicio3_$T.txt | \
	sed 's/ir\[t[0-9][^;]*;\s//g' | \
	sed 's/sfpr\[f[4-9][^;]*;\s//g' | \
	sed 's/sfpr\[f10[^;]*;\s//g' | \
	sed 's/dfpr[^;]*;\s//g' | \
	...
	sed 's/ir\[at[^;]*;\s//g' | \
	sed 's/ir\[fp[^;]*;\s//g' | \
	sed 's/ir\[a[0-7][^;]*;\s//g' > solution/output/output_ejercicio3_$T.txt 

	rm solution/output/aux_output_ejercicio3_$T.txt
done
